...
public class aAppointmentsController implements Initializable {
    ...
    public void setAppointmentTableData() {
        try {
            ...
            while (rs.next()) {
                aAppointmentList.add(new AppointmentTable(rs.getString("Name"), rs.getString("UserName"), rs.getString("Date"), rs.getString("Time"), rs.getString("Phone"), rs.getString("Doctor"), rs.getString("Injury_or_Condition")));
            }
            //add to table
